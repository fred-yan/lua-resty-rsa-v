...
-- All rights reserved.
-- User: fredyan <fredyan018@gmail.com>
-- Date: 2020/7/12
--
...
function _M.decrypt(self, str)
    local rsa = self.rsa
    if not rsa then
        return nil, "inited failed for public decrypt"
    end

    local keysize = C.RSA_size(rsa)
    local rsa_out = ffi.new("char[?]", keysize)

    local c_str = ffi.new("char[?]", #str+1)
    ffi.copy(c_str, str)

    local rsa_outlen = C.RSA_public_decrypt(#str, c_str, rsa_out, rsa, PADDING.RSA_PKCS1_PADDING)
    if rsa_outlen <=0 then
        return ssl_err()
    end

    return ffi_str(rsa_out, rsa_outlen)
end

function _M.encrypt(self, str)
    local rsa = self.rsa
    if not rsa then
        return nil, "inited failed for pricate decrypt"
    end
    ...
    local rsa_outlen = C.RSA_private_encrypt(#str, c_str, rsa_out, rsa, PADDING.RSA_PKCS1_PADDING)
    if rsa_outlen <=0 then
        return ssl_err()
    end
